...
.PARAMETER GetPassword
    Specifies that you wish to retrieve an existing password
    -CredType may be required to access the correct credential if more than one is
    specified for a target
...
(
    ...
    [Parameter(Mandatory=$false)][Switch] $GetPassword,
...
#region Reading selected credential
    if($GetPassword)
    {
        ...
        [String] $CredStr = "$($Cred.CredentialBlob)"
        Write-Host $CredStr
    }
#endregion
